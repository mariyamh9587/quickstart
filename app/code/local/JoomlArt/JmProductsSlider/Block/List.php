...

{
	...
	public function __construct($attributes = array()){
		...
		$devicedetect = Mage::helper ( 'joomlart_jmproductsslider/mobiledetect' );
		...
		$this->_config["circular"] = true;

		$this->_config['qty'] = $helper->get('quanlity', $attributes);

		$qtytablet = $helper->get('quanlitytablet', $attributes);

		$qtymobile = $helper->get('quanlitymobile', $attributes);

        

		if($devicedetect->isTablet()){

			  $this->_config ['qty'] = $qtytablet;

			  

	    }elseif($devicedetect->isMobile()){

              $this->_config["circular"] = false;

              $this->_config ['qty'] = $qtymobile ;
		...
		if(is_null($this->_productCollection)){
	      	...
	      	switch ($this->_config['mode']){
				...
				case 'discount':
					...
					$listall = $this->getDiscountProducts();
